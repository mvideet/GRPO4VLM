            ...
            for sample in data_generator:
                with self.accelerator.accumulate(self.policy):
                    ...
                    if self.beta > 0 and self.ref_model is not None:
                        ...
                        # KL(P||Q) = P(x) * log(P(x)/Q(x)) = exp(log_p) * (log_p - log_q)
                        # where log_p = ref_action_log_probs, log_q = action_log_probs
                        per_token_kl = torch.exp(ref_action_log_probs) * \
                                      (ref_action_log_probs - action_log_probs)
